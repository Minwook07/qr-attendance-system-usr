...
function ActionButton({ label, color, icon, onClick }) {
    return (
        <button
            onClick={onClick}
            className={`${color} hover:opacity-90 text-white font-medium px-4 py-2 rounded-md flex items-center gap-2 transition-opacity`}
        >
...
function AttendanceTable({ data }) {
    return (
        <div className="overflow-x-auto">
            <table className="min-w-full bg-white border border-gray-300 rounded-lg" id="attendance-table">
                <thead className="bg-gray-800 text-white">
                    <tr>
                        ...
                        <th className="px-4 py-3 text-left">Work Hour</th>
                        <th className="px-4 py-3 text-left">Over Time</th>
                        <th className="px-4 py-3 text-left">Late Time</th>
                        <th className="px-4 py-3 text-left">Early Out</th>
                        <th className="px-4 py-3 text-left">In Location</th>
                        <th className="px-4 py-3 text-left">Out Location</th>
                    </tr>
                </thead>
                <tbody>
                    {data.map((row, index) => (
                        <tr key={row.id} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                            <td className="px-4 py-3">
                                <img src={row.image} alt={row.name} className="w-12 h-12 rounded-full object-cover" />
                            </td>
                            ...
                            <td className="px-4 py-3">{row.workHour}</td>
                            <td className="px-4 py-3">{row.overTime}</td>
                            <td className="px-4 py-3">{row.lateTime}</td>
                            <td className="px-4 py-3">{row.earlyOutTime}</td>
...
export default function AttendanceContent() {
    ...
    return (
        <div className="p-6 bg-gradient-to-br from-gray-900 to-gray-800 min-h-screen text-white">
            <h2 className="text-4xl mb-6 font-bold">Attendance Content</h2>
